...
use session::RabbitMqSession;
use session_handle::RabbitMqSessionHandle;
...
use warpgate_tls::{
    MaybeTlsStream, ResolveServerCert, ServerTlsStream, TlsCertificateAndPrivateKey,
};

pub struct RabbitMqProtocolServer {
    services: Services,
}

impl RabbitMqProtocolServer {
...
impl ProtocolServer for RabbitMqProtocolServer {
    async fn bind(
        self,
        ...
        // Like Redis, AMQP 0-9-1 has no in-protocol STARTTLS: a listener
        ...
        Ok(async move {
            loop {
                ...
                tokio::spawn(async move {
                    ...
                    let (session_handle, mut abort_rx) = RabbitMqSessionHandle::new();
                    ...
                    let session = RabbitMqSession::new(
                        server_handle,
                        services,
                        wrapped_stream,
                        remote_address,
...
impl Debug for RabbitMqProtocolServer {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("RabbitMqProtocolServer").finish()
    }
}
