...
/// Thin compatibility wrapper: cluster auth itself lives in `warpgate-tunnel-client`
/// so that other protocols tunneling through a Kubernetes target's port-forward
/// API can build the same authenticated client without depending on this
/// crate's reverse-proxy server code.
pub async fn create_authenticated_client(
    k8s_options: &TargetKubernetesOptions,
    auth_user: Option<&String>,
    _services: &Services,
) -> anyhow::Result<reqwest::ClientBuilder> {
    warpgate_tunnel_client::kubernetes::auth::create_authenticated_client(
        k8s_options,
        auth_user.map(|s| s.as_str()),
    )
        .await
}
